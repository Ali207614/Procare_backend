exports.up = async function (knex) {
  const hasWarrantyPeriod = await knex.schema.hasColumn('repair_parts', 'warranty_period');
  if (!hasWarrantyPeriod) {
    await knex.schema.alterTable('repair_parts', (table) => {
...
exports.down = async function (knex) {
  ...
  if (hasHistoryTable) {
    await knex('history_tracked_fields')
      .where({ entity_table: 'repair_parts', field_path: 'warranty_period' })
      .delete();
  }
