WITH filtered AS (
    ...
    WHERE
      ...
      -- EXCLUDE branch
      AND (
        :exclude_branch_ids::uuid[] IS NULL OR
          NOT EXISTS (
              SELECT 1
              FROM admin_branches ab3
              WHERE ab3.admin_id = a.id
                AND ab3.branch_id = ANY(:exclude_branch_ids::uuid[])
          )
        )
      ...
      -- EXCLUDE role
      AND (
        :exclude_role_ids::uuid[] IS NULL OR
          NOT EXISTS (
              SELECT 1
              FROM admin_roles ar3
              WHERE ar3.admin_id = a.id
