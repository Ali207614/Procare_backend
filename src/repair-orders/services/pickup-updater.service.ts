...
export class PickupUpdaterService {
  ...
  async create(
    ...
    const [result] = await this.knex.transaction(async (trx) => {
      ...
      const [inserted]: RepairOrderPickup[] = await trx('repair_order_pickups')
        .insert(row)
        .returning('*');

      await this.changeLogger.logIfChanged(trx, orderId, 'pickup', null, inserted, admin.id);
  ...
  async update(
    ...
    await this.knex.transaction(async (trx) => {
      const old: RepairOrderPickup | undefined = await trx<RepairOrderPickup>(
      ...
      await trx('repair_order_pickups').where({ id: pickupId }).update(updatedRow);
  ...
  async delete(pickupId: string, admin: AdminPayload): Promise<{ message: string }> {
    ...
    await this.knex.transaction(async (trx) => {
      ...
      await trx('repair_order_pickups').where({ id: pickupId, status: 'Open' }).update({
        status: 'Deleted',
        updated_at: trx.fn.now(),
        updated_by: admin.id,
      });

      await this.changeLogger.logIfChanged(trx, old.repair_order_id, 'pickup', old, null, admin.id);
    });
