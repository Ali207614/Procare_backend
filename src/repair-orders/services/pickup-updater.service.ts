...
export class PickupUpdaterService {
    ...
    async create(orderId: string, pickup: any, adminId: string) {
        ...
        if (!status) {
            throw new BadRequestException({ message: 'Repair order not found', location: 'repair_order_id' });
        }
    ...
    async update(orderId: string, pickup: any, adminId: string) {
        ...
        const old = await this.knex('repair_order_pickups')
            .where({ repair_order_id: orderId })
            .first();

        await this.knex('repair_order_pickups')
            .where({ repair_order_id: orderId })
            .delete();
